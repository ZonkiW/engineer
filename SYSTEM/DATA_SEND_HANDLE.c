...
void data_pack_handle(uint16_t cmd_id, uint8_t *p_data, uint16_t len)
{
  ...
  frame_header_t *p_header = (frame_header_t*)computer_tx_buf;//帧头
  p_header->sof          = UP_REG_ID;
  p_header->data_length  = len;
  Append_CRC8_Check_Sum(computer_tx_buf, HEADER_LEN);
	
  memcpy(&computer_tx_buf[HEADER_LEN], (uint8_t*)&cmd_id, CMD_LEN);//命令码
    
  memcpy(&computer_tx_buf[HEADER_LEN + CMD_LEN], p_data, len);//数据
	
  Append_CRC16_Check_Sum(computer_tx_buf, HEADER_LEN + CMD_LEN + len + CRC_LEN); //校验码
}                               
